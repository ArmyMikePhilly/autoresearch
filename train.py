...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        if fa3 is not None:
            ...
            y_pt = F.scaled_dot_product_attention(q_pt, k_pt, v_pt, is_causal=True)
            y = y_pt.transpose(1, 2)
...
if torch.cuda.is_available():
    adamw_step_fused = torch.compile(dynamic=False, fullgraph=True)(_adamw_step_fused_impl)
    muon_step_fused = torch.compile(dynamic=False, fullgraph=True)(_muon_step_fused_impl)
else:
...
if device.type == "cuda":
    ...
    print("Bypassing torch.compile on CPU...")
